...
public class PolicyListenerSimple {
	...
	public void initMatchers() {
		try {
			...
			userActivatedRuleMatcher = UserActivatedRoleRuleMatcher.FACTORY
					.getMatcher(policy);
	...
	public PolicyListenerSimple(PolicyTextualEditor edit) {
		editor = edit;
		...
		monitorActivatedUserRule = userActivatedRuleMatcher
				.newDeltaMonitor(false);
	}

	public PolicyListenerSimple(SimulationSimple s) {
		simulation = s;
		...
		permissionMatcher.addCallbackAfterUpdates(new Runnable() {
			...
			public void run() {
				for (PermissionSignature sig : monitorPermission.matchFoundEvents) {
					String permission = ((Permission) sig.getValueOfP())
							.getName();
		...
		operationMatcher.addCallbackAfterUpdates(new Runnable() {
			...
			public void run() {
				for (OperationSignature sig : monitorOperation.matchFoundEvents) {
					String operation = ((Operation) sig.getValueOfOP())
							.getName();
		...
		objectMatcher.addCallbackAfterUpdates(new Runnable() {
			...
			public void run() {
				...
				for (ObjectSignature sig : monitorObject.matchFoundEvents) {
					String object = ((policy.Object) sig.getValueOfOB())
							.getName();
		...
		userRuleMatcher.addCallbackAfterUpdates(new Runnable() {
			...
			public void run() {
				for (UserRoleRuleSignature sig : monitorUserRule.matchFoundEvents) {
					...
					// String script = getTransformator().addUserRule(user,
					// operation, object);
					// applyScript(script);
		...
		userActivatedRuleMatcher.addCallbackAfterUpdates(new Runnable() {
			...
			public void run() {
				for (UserActivatedRoleRuleSignature sig : monitorActivatedUserRule.matchFoundEvents) {
					String user = ((policy.User) sig.getValueOfUSER())
							.getName();
					String operation = ((policy.Operation) sig
							.getValueOfOPERATION()).getName();
					String object = ((policy.Object) sig.getValueOfOBJECT())
							.getName();
					String role = ((policy.Role) sig.getValueOfROLE())
							.getName();
					String script = getTransformator().addUserRule(user,
							operation, object);
					applyScript(script);
	...
	public void verifyEnforcement(String policyChange) {
		...
		while (shouldContinue) {
			for (Object[] sigX : getPolicyRulesActivated()) {
				...
				for (Object[] sigX2 : simulation.kevoreeListener
						.getPolicyRulesEnforced()) {
					...
					if (userXX.equals(userXX2) && roleXX.equals(roleXX2)
							&& operationXX.equals(operationXX2)
							&& objectXX.equals(objectXX2)) {
						isEnforced = true;
					}
				}
				if (!isEnforced) {
		...
		// editor.outputEditor.consoleEditor.print(policyChange
		// +" "+rulesEnforced);
	}

	public java.lang.Object[][] getPolicyRules() {
		java.lang.Object[][] data = new java.lang.Object[userRuleMatcher
				.getAllMatchesAsSignature().size()][4];
		int cpt = 0;
		for (UserRoleRuleSignature rule : userRuleMatcher
				.getAllMatchesAsSignature()) {
			String userX = ((policy.User) rule.getValueOfUSER()).getName();
			String operationX = ((policy.Operation) rule.getValueOfOPERATION())
					.getName();
			String objectX = ((policy.Object) rule.getValueOfOBJECT())
					.getName();
	...
	public java.lang.Object[][] getPolicyRulesActivated() {
		java.lang.Object[][] data = new java.lang.Object[userActivatedRuleMatcher
				.getAllMatchesAsSignature().size()][4];
		int cpt = 0;
		for (UserActivatedRoleRuleSignature rule : userActivatedRuleMatcher
				.getAllMatchesAsSignature()) {
			String userX = ((policy.User) rule.getValueOfUSER()).getName();
			String operationX = ((policy.Operation) rule.getValueOfOPERATION())
					.getName();
	...
	public String removeRule(String userName, String operationName,
			String objectName) {
		Policy2KevScript policy2KevScript = new Policy2KevScript(policy);
		String res = "";
		int haveToSuppressChannel = 0;
		for (UserRoleRuleSignature sig : userRuleMatcher
				.getAllMatchesAsSignature()) {
			String user = ((User) sig.getValueOfUSER()).getName();
			String operation = ((Operation) sig.getValueOfOPERATION())
					.getName();
			if (user.equals(userName) && operation.equals(operationName)) {
				res = res
						+ "\n"
						+ policy2KevScript
								.removeUserRuleBindingChannelObjectOperation(
										userName, operationName, objectName);
				return res;
			}
		}
		res = res
				+ "\n"
				+ policy2KevScript.removeUserRule(userName, operationName,
						objectName);
		return res;
	}
	...
	public void applyScript(String s) {
		...
		// KevScriptEngine kse =
		// editor.kevGuard.getKevScriptEngineFactory().createKevScriptEngine();
		// kse.append(s);
